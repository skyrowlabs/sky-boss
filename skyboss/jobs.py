...
def load(home: Path | None = None) -> tuple[list[Job], list[str]]:
    ...
    from skyboss.tools import walk
...
def calendar(schedule: str) -> tuple[str, str]:
    """`(normalized, "")` or `("", reason)`, decided by systemd.

    **Nothing here parses a calendar syntax.** `systemd-analyze` owns
    `OnCalendar` the way cron owns `15 5 * * *`, and a second implementation
    beside either would be wrong about DST before it was wrong about anything
    else. Also: a machine with no `systemd-analyze` cannot install, and says so
    rather than writing a unit nobody validated.
    """
    if not schedule.strip():
        return "", "no schedule declared — a manual job has nothing to install"
    try:
        done = subprocess.run(
        ...
        return "", "systemd-analyze is not available — cannot validate a schedule"
    if done.returncode != 0:
        detail = (done.stderr or done.stdout).strip().splitlines()
        return "", f"systemd rejected {schedule!r}: {detail[0] if detail else 'invalid'}"
    for line in done.stdout.splitlines():
        if line.strip().startswith("Normalized form:"):
            return line.split(":", 1)[1].strip(), ""
    return schedule, ""


def service_unit(job: Job, normalized: str) -> str:
    """The unit that runs the job.

    **`ExecStart` is `sb job run <name>`, never the job's own argv**, and that is
    the difference between a scheduler and a cron line. Going through the
    command means a timed run takes the lane, honours the timeout and writes the
    ledger exactly as a manual one does. A unit that ran the argv directly would
    bypass all three, silently, for precisely the runs nobody is watching.
    """
    return f"""# Generated by sky.boss from {home_file()} — `sb job install {job.name}`.
# Edits here are overwritten on the next install and are invisible to `sb job list`.
[Unit]
Description={job.description or f"sky.boss job {job.name}"}

[Service]
Type=oneshot
ExecStart={sb_executable()} job run {job.name}
"""


def timer_unit(job: Job, normalized: str) -> str:
    """The clock. `WantedBy=timers.target` is the enable mechanism every timer
    uses, and is the only unit name outside the `sb-` prefix anything here
    writes — asserted by a test.

    **No `Persistent=`.** It would fire a missed run at the next boot, which
    cron does not do — and these jobs are being migrated off cron, so matching
    what they did is the less surprising default. A catch-up is a declared key
    if anyone wants one, not a behaviour that arrives unannounced.
    """
    return f"""# Generated by sky.boss from {home_file()} — `sb job install {job.name}`.
# Edits here are overwritten on the next install and are invisible to `sb job list`.
[Unit]
Description={job.description or f"sky.boss job {job.name}"} (timer)
...
def payload(job: Job) -> str:
    """The work a job does, as a string, with sky.boss's own wrapper removed.

    `["run", "--", "jam", "report", "overnight"]` is `jam report overnight` —
    which is what the operator's crontab line says too, and is why a collision
    can be found without either side's clock being parsed.
    """
    rest = job.argv[1:]
    if rest and rest[0] == "--":
        rest = rest[1:]
...
def busy_lines() -> tuple[list[tuple[str, str]], list[str]]:
    """Everything already scheduled on this machine, as opaque text.

    **Never parsed for meaning and never written.** A crontab entry is a line;
    a foreign unit is a file. Both are read for one purpose — does something
    already run this work — and for nothing else.

    Scope is stated rather than implied: the invoking user's crontab and the
    user units in `XDG_CONFIG_HOME`. System units, other users' crontabs and
    anything started by another supervisor are outside it, and `sb job install`
    says so when it reports a clean check.
    """
        ...
        if done.returncode == 0:
            ...
            for line in done.stdout.splitlines():
                if line.strip() and not line.lstrip().startswith("#"):
                    lines.append(("crontab", line.strip()))
    except (OSError, subprocess.SubprocessError):
        pass

    try:
        units = sorted(units_dir().glob("*.service"))
        scopes.append("user units")
    except OSError:
        units = []
    for unit in units:
        ...
        try:
            for line in unit.read_text().splitlines():
                if line.strip().startswith("ExecStart"):
                    lines.append((unit.name, line.strip()))
        except OSError:
            continue
    return lines, scopes


def collisions(job: Job) -> tuple[list[tuple[str, str]], str]:
    """What already runs this job's work, and why the check may be worthless.

    **The check is on the command, not the clock**, and that is a correction to
    how this was first specced. The hazard is not *two things fire at 02:00* —
    it is *the same work runs twice*, which is what a half-finished handover
    leaves behind. Comparing commands also needs no calendar parsed on either
    side, where comparing times would need cron parsed on one.

    A payload too short to be distinctive returns *cannot check* rather than
    *clean*: matching `true` against every crontab line would find a collision
    in somebody's `PATH`, and reporting no match for it would be worse — a
    detector that counts what it caught and never counts whether it could look
    answers `0` for both.
    """
    work = payload(job)
    if len(work) < 6 or " " not in work:
        return [], f"{work!r} is too short to check against other schedules"
    lines, _ = busy_lines()
    return [(where, line) for where, line in lines if work in line], ""


@job.command(name="install")
@click.argument("name")
@click.option("--force", is_flag=True, help="Install even if something already runs this work.")
@emit
def job_install(name: str, force: bool) -> Result:
    """Generate this job's systemd units. **Does not enable them.**

        sb job install nightly

    **This acts** — it writes two files into `~/.config/systemd/user/` and
    reloads the user manager. Generating never enables, so nothing fires until
    you run the `systemctl --user enable` line this prints.

    It refuses if something on this machine already runs the same work, naming
    what it found; `--force` installs alongside it.
    """
    result = Result()
    jobs, problems = load()
    for problem in problems:
    ...
    if trouble:
        result.ok = False
        result.data = {"job": name, "error": trouble}
        return result

    clashes, unchecked = collisions(found)
    if clashes and not force:
        result.ok = False
        result.data = {
            "job": name,
            "error": "something already runs this work",
            "found": [{"where": w, "line": ln} for w, ln in clashes],
        }
        one = len(clashes) == 1
        result.warn(
            f"refused: {len(clashes)} existing "
            f"{'entry already runs' if one else 'entries already run'} "
            f"{payload(found)!r} — deactivate "
            f"{'it' if one else 'them'} first, or --force to install alongside"
        )
        return result
    if clashes:
        result.warn(
            f"installed alongside {len(clashes)} existing "
            f"{'entry' if len(clashes) == 1 else 'entries'} — both will fire"
        )

    target = units_dir()
    target.mkdir(parents=True, exist_ok=True)
    (target / found.service).write_text(service_unit(found, normalized))
    (target / found.timer).write_text(timer_unit(found, normalized))
    _systemctl("daemon-reload")

    if unchecked:
        # A check that could not run says so rather than reading as clean.
        result.warn(f"collision not checked — {unchecked}")
    result.data = {
        "job": name,
        "service": str(target / found.service),
        "timer": str(target / found.timer),
        "schedule": found.schedule,
        "normalized": normalized,
        "enabled": False,
        "enable_with": f"systemctl --user enable --now {found.timer}",
    }
    result.warn(f"generated, not enabled — `systemctl --user enable --now {found.timer}`")
    return result


@job.command(name="uninstall")
@click.argument("name")
@emit
def job_uninstall(name: str) -> Result:
    """Disable and remove this job's systemd units.

        sb job uninstall nightly

    **This acts.** It touches only `sb-<name>.service` and `sb-<name>.timer`,
    and works for a job whose declaration is already gone — which is how an
    orphan gets cleaned up.
    """
    result = Result()
    if not NAME.match(name):
        raise click.UsageError("name must be lowercase letters, digits and hyphens")

    timer, service = f"{PREFIX}{name}.timer", f"{PREFIX}{name}.service"
    ...
    for unit in (timer, service):
        ...
        try:
            ...
            result.ok = False
            result.data = {"job": name, "error": f"{unit}: {exc}"}
            return result
    _systemctl("daemon-reload")

    result.data = {"job": name, "removed": removed}
    if not removed:
        result.warn(f"nothing to remove — no {timer} or {service}")
    return result
...
# by reading the catalog rather than by anything failing. `tests/test_jobs.py`
# now recomputes this from the help text so the next one cannot slip.
# Set on **every** subcommand, including the observes, because a missing
# attribute and a declared `False` are the same value to `getattr` and only one
