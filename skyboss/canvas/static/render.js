 ...
 * skyboss/theme.py names a colour holds in JavaScript too.
 */
 ...
 *
 * This is the one piece of `skyboss/view.py` that has a second implementation, and
...
/* `_EMPTY` in skyboss/view.py, plus zero. Written out rather than inlined because
 ...
 * `skyboss/view.py`, deliberately, because this file has no test runner. */
...
/* `_plural` in skyboss/output.py. Four words rather than a shipped string, for the
 * same reason the count itself is arithmetic here. */
function plural(n, word) {
  return `${n} ${word}${n === 1 ? "" : "s"}`;
}

/* `_dimensions` in skyboss/output.py, mirrored for the same reason `plural` is:
 ...
 * `view_for` in skyboss/view.py — a lookup, not a judgment. */
 ...
 * is was decided in `skyboss/view.py`; this only obeys. See [[table-views]] round 4.
 *
 ...
 * which is the boundary tests/test_chrome.py and skyboss/view.py both hold.
 *
 ...
 * word-for-word in step with `_dimensions` in skyboss/output.py. */
