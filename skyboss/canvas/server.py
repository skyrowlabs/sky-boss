...
def build(canvas: Canvas | None = None) -> Starlette:
    ...
    async def post_tools(request: Request) -> Response:
        ...
        **Reloads the tree after writing**, because the alternative is a
        ...
        from skyboss import cli as root_group
    ...
    async def get_vocabulary(request: Request) -> Response:
        ...
        if not canvas.authorised(request):
            return _denied()
        from skyboss.canvas.catalog import vocabulary
    ...
    async def get_projects(request: Request) -> Response:
        ...
        *declaration*, which is true whether or not the command has been run.
        ...
        if not canvas.authorised(request):
            return _denied()
        from skyboss.rollcall import load
...
def resolve_follow(argv: list[str], root=None) -> Follow:
    ...
    from skyboss.follow import is_file_form
...
def _accrual(follower, child, code: int | None, now: float):
    ...
    """
    from skyboss.read import envelope_for as read_envelope
    from skyboss.run import envelope_for as run_envelope
