...
def test_the_known_keys_cover_every_field_a_project_can_carry():
    ...
    *assigned* rather than declared — it is sky.boss's own answer about which
    ...
    derived = {"name", "shade"}
    expected = {"from" if f.name == "from_" else f.name for f in fields(Project) if f.name not in derived}
...
def invoke(args=()):
    ...
    assert result.stdout.strip(), (
        f"`sb --json roll-call {' '.join(args)}` printed no envelope on stdout "
...
def refuse(args=()):
    ...
    return CliRunner().invoke(cli, ["--json", "roll-call", *args])
