...
def view_of(*args, **kwargs) -> dict:
    """`shape`, asserted non-None.

    `shape` returns `dict | None`, and the None is a real contract — *nothing to
    say, render as you always did*. Two tests below assert exactly that and call
    `shape` directly. Every other test here is about what it **does** say, so
    the assumption is stated once here rather than left to a `TypeError` at the
    first subscript. See `tests/narrowing.py`.
    """
    return present(shape(*args, **kwargs), "a view")
...
def test_short_hex_is_not_an_opaque_id():
    ...
    assert "code" in keys(view_of(rows))
...
def test_a_short_string_column_stays_inline(rows):
    ...
    view = view_of([{"title": "CLEAN"}, {"title": "DIRTY"}])
...
def test_explicit_columns_still_work_with_shaping_declined(rows):
    ...
    view = view_of(rows, cols=["number"], enabled=False)
...
def test_shaping_never_touches_the_data(rows):
    ...
    view_of(rows)
    view_of(rows, cols=["checks.failed"])
    view_of(rows, drop=["title"])
    assert rows == before
...
def test_a_column_missing_from_some_rows_is_still_seen():
    ...
    assert "error" in keys(view_of(rows))
...
def test_the_header_floor_is_capped(rows):
    ...
    view = view_of([{"a_really_quite_long_field_name": 1, "b": 2}])
...
def test_find_rows_refuses_to_choose_between_two_candidates():
    ...
    reason = present(found.reason, "a reason")
    assert "jobs" in reason and "errors" in reason
    assert "--rows" in present(found.reason, "a reason")
...
def test_find_rows_finds_the_only_row_list(wrapped):
    ...
    assert found.key == "jobs"
    assert len(present(found.rows, "rows")) == len(wrapped["jobs"])
...
def test_a_named_column_no_row_carries_is_reported(rows):
    view = view_of(rows, cols=["number", "nope"])
...
def test_a_field_that_exists_and_is_empty_is_not_missing():
    ...
    view = view_of([{"a": 1, "b": None}, {"a": 2, "b": None}], cols=["a", "b"])
...
def test_a_field_only_some_rows_carry_is_not_missing():
    ...
    view = view_of([{"a": 1}, {"a": 2, "b": 3}], cols=["b"])
    assert "missing" not in view


def test_a_dotted_path_that_resolves_nowhere_is_missing(rows):
    assert view_of(rows, cols=["checks.nope"])["missing"] == ["checks.nope"]


def test_a_dotted_path_that_resolves_is_not_missing(rows):
    assert "missing" not in view_of(rows, cols=["checks.passed"])
...
def test_missing_is_omitted_rather_than_empty(rows):
    ...
    assert "missing" not in view_of(rows, cols=["number"])
...
def test_the_heuristic_never_reports_missing(rows):
    ...
    assert "missing" not in view_of(rows)
...
def test_a_column_no_row_has_is_never_offered():
    ...
    *name* one and it is drawn empty on purpose, but it never joins the set you
    ...
    rows = [{"a": 1, "b": 2}]
    assert "nope" not in offered(view_of(rows))
    named = view_of(rows, cols=["a", "nope"])
    ...
    assert offered(view_of(rows)) == ["a", "b"]
...
def test_a_filtered_view_answers_a_narrower_question():
    ...
    rows = [{"a": 1, "b": 2, "c": 3}]
    filtered = view_of(rows, cols=["a"])
    assert filtered["hidden"] == []
    assert offered(filtered) == ["a"]
    assert offered(view_of(rows)) == ["a", "b", "c"]
