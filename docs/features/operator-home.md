...
3. **Move to `~/skyrow.labs/tackle-box`**, then add the remote.
...
deliberately outside `~/skyrow.labs`, because it is not a product."* Adding a remote and making it
shareable is what retires that premise. It also collapses `unpushed.DEFAULT_ROOTS` from two roots
to one, since `SCAN_DEPTH = 2` finds `~/skyrow.labs/tackle-box` on its own — so `tb check
...
- [x] CLAUDE.md's "deliberately outside `~/skyrow.labs`" reversed, with the reason it expired
...
- [ ] Move to `~/skyrow.labs/tackle-box`
