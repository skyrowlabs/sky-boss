...
- [ ] **`--delay` on `run`**, drawing through `resident.hold` so `q`, `Esc` and Ctrl-C cancel by
...
- [ ] **Refused together with `--refresh`**, with a message that names the reason rather than
      the rule. Composed with `--json`: countdown on stderr, one envelope on stdout, proven by
      the purity test that already exists.
- [ ] **Help is the doc.** The runnable example, the cancel keys, and the sentence that says a
