...
The ideas list asks, of chaining tb into an agent: *"how different is this from the tools we
have?"* Honestly answered: **barely, and the arrow is pointing the wrong way.**

`tb run -- claude -p "…"` already works. `tb follow -- claude -p "…"` already works and gives
you a ring, a liveness clock and a dead state. Feeding tb's output *into* an agent is
`tb --json data -- jam pr list --json | claude -p "…"`, which is a shell pipe, and `tools.toml`
...
**The direction with value is the inverse: let the agent call tb.** Today an agent that wants to
...
`tools.toml`. `tb tools jam-pr-list` is a name that means *"open PRs, with the merge column
GitHub cannot show, from the right directory, as data."* That is exactly the shape MCP exists to
carry, and tb is already holding it.
...
2. **`--json` purity is already a tested invariant.** Over stdio, the MCP protocol *is* stdout —
   so the rule that keeps `tb --json … | jq` parseable is the same rule that keeps the protocol
...
### The toolbox is the surface, and this is the whole safety argument

**Exposing `data` or `read` would convert an operator's assertion into an agent's guess.** The
act/observe split rests on one sentence: *tb cannot tell a read from a write by inspecting an
...
- **`resident` → excluded.** A stream has no single response, which is why `tb follow` already
  refuses `--json`; a request/response protocol is the same shape of problem and gets the same
  answer.
- **tb's own commands → excluded.** `tb tools` is a listing, and MCP lists tools natively; the
  argv-takers are excluded above. `tb mcp` excludes itself with `tb_surface`, exactly as
  `tb ui` does.
...
The entire threat model collapses into "who may spawn `tb mcp`", which the operating system
already answers.
...
Unchanged, and this is why the boundary above matters. **tb is never in the credential path** —
...
- **No second description language.** What the agent reads is the `description` the operator
  wrote in `tools.toml`, already used as `short_help` and already shown in `tb tools`. One
...
design — one MCP tool per tb command — and points at the toolbox, where every argv already has
a human behind it.
...
only as useful as the toolbox is full, and until [[tools]] round 3 filling the toolbox meant
opening `$EDITOR` and retyping an argv. `--save` made curation cost one flag at the moment the
command already worked. This round is what that curation is *for* — the toolbox stops being a
